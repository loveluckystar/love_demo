...
public class HttpSessionSidWrapper extends HttpSessionWrapper {
  ...
  private String sid = "";
  private static int EXPIRE_SECONDS = 3600;//最大闲置时间60分钟
  
  public static String SESSIONID = "SP_MY_JSESSIONID";//cookie的key
...
//      RedisManager.expire(this.sid, EXPIRE_SECONDS);//重置session缓存的有效时间
//      return value;
//    }
    try {// redis 获取session
...
//      logger.info("session_not_null:" + this.sid + ":" + name + ":" + super.getMaxInactiveInterval());
      ...
      this.setAttribute(name, value);//设置session到本地服务器     
      return value;
    } catch (Exception ex) {
